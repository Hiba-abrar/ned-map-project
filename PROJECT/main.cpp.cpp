...
vector<double> dijkstra(int n, const vector<vector<Edge>> &graph, int src, vector<int> &parent)
{
    ...
    while (!pq.empty())
    {
        ...
        // Skip if current node is closed (except source)
        ...
        for (const auto &e : graph[u])
        {
            ...
            // Skip if destination node is closed
            if (is_node_closed(v, graph))
                continue;
...
void navigation_mode(const unordered_map<int, Node> &nodes, const vector<vector<Edge>> &graph)
{
    ...
    // STEP 1: Get GPS location (mandatory)
    ...
    // Validate path doesn't include closed nodes
    for (int i = 1; i < path.size() - 1; i++) // Skip start and end nodes
    {
        if (is_node_closed(path[i], graph))
        {
            speak("Route calculation error: Path includes closed location. Recalculating...");
...
cd /Users/eeshahrashid/project_folder
g++ -std=c++17 test3.cpp -o main && ./main
